...
def evaluate_model(model, test_dataloader, device):
    ...
    with open(keywords_file, 'r', encoding='utf-8') as run_file:
        for line in run_file:
            ...
            if len(parts) >= 2:  # Ensure the line has at least a document ID and keywords
                # Extract the keywords (assuming they are in the second column)
                keywords = parts[1].replace(',', '')  # Replace commas with spaces
                keywords_list.append(keywords)

    loop = tqdm(test_dataloader, leave=True, ncols=100)
    ...
    for idx, batch in enumerate(loop):
        ...
        for generated_text in generated_texts:
            ...
            grouped_generated_tokens = [' '.join(generated_tokens[i:i+5]) for i in range(0, len(generated_tokens), 5)]
...
for epoch in range(epochs):
    loop = tqdm(train_dataloader, leave=True, ncols=100)
    step_counter = 0
    total_loss = 0 
