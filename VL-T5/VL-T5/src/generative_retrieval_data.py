...
class GRFineTuneDataset(Dataset):
    def __init__(self, split='train', raw_dataset=None, rank=-1, topk=-1, verbose=True, args=None, mode='train'):
        ...
        source = self.source
        data_info_path = dataset_dir.joinpath(f'./Final/Full_turn_query/{source}_dataset_bert_full_turn.json')
        n_images = 0
        with open(data_info_path) as f:
            ...
            if source == 'train':
                for k in karpathy_data:
                    ...
                    new_datum = {
                        # 'img_ids': (
                        #     k['img_ids1'][:3]  #+ # Take the first 3 elements from img_ids1
                        #     # k['img_ids2'][:3] +  # Take the first 3 elements from img_ids2
                        #     # k.get('img_ids3', [])[:3] +  # Safely take up to 3 elements from img_ids3 (if it exists)
                        #     # k.get('img_ids4', [])[:3]    # Safely take up to 3 elements from img_ids4 (if it exists)
                        # ),
                        'img_ids': (
                            k.get('img_ids4', k.get('img_ids3', k.get('img_ids2', [])))[2:]  # Select up to 3 elements
                        ),
                        'id': k['facet_id'],
                        'sent': (
                            k['topic'] + ' ' +
                            k['question1'] #+ ' ' + k['answer1'] + ' ' +
                            # k['question2'] + ' ' + k['answer2'] + ' ' +
                            # k.get('question3', '') + ' ' + k.get('answer3', '') + ' ' +
                            # k.get('question4', '') + ' ' + k.get('answer4', '')
                        ).strip(),
                        'tag': k['tag'],
                        'targets': k['related_dict'],
                        'unrelated_dict': k['unrelated_dict'],
                        'is_train': True,
                    }
                    
                    # print(new_datum['img_ids1'])
                    # raise RuntimeError
        ...
        # self.source_to_h5 = {}
        self.dict_topic = pickle.load(open(f'{dataset_dir}/keywords_output_new.pkl','rb'))
    ...
    def __getitem__(self, idx):
        ...
        if 'reference' in datum:
            out_dict['reference'] = datum['reference']

        if 'targets' in datum:
            # out_dict['targets'] = ' [SEP] '.join([self.dict_topic[t] for t in datum['targets']])
            out_dict['targets'] = ' [SEP] '.join([
                self.dict_topic[t] if t in self.dict_topic else self.dict_topic[t + " "]
                for t in datum['targets'] if t in self.dict_topic or t + " " in self.dict_topic
            ])
            targets = out_dict['targets']

        if 'unrelated_dict' in datum:
            out_dict['unrelated_dict'] = ' [SEP] '.join([
                self.dict_topic[t] if t in self.dict_topic else self.dict_topic[t + " "]
                for t in datum['unrelated_dict'] if t in self.dict_topic or t + " " in self.dict_topic
            ])
    
            unrelated_dict = out_dict['unrelated_dict']
        ...
        if datum['is_train']:
            ...
            unrelated_dict = unrelated_dict.strip()
            if 't5' in self.args.tokenizer:
                unrelated_dict_ids = self.tokenizer.encode(unrelated_dict, max_length=self.args.gen_max_length, truncation=True)
            elif 'bart' in self.args.tokenizer:
                unrelated_dict_ids = self.tokenizer.encode(unrelated_dict, max_length=self.args.gen_max_length, truncation=True)

            assert len(unrelated_dict_ids) <= self.args.gen_max_length, len(unrelated_dict_ids)
            # out_dict['sent'] = sent
            out_dict['unrelated_dict_ids'] = torch.LongTensor(unrelated_dict_ids)
            out_dict['unrelated_dict_length'] = len(unrelated_dict_ids)
    ...
    def collate_fn(self, batch):
        ...
        if 'unrelated_dict_ids' in batch[0]:
            T_W_L = max(entry['unrelated_dict_length'] for entry in batch)
            unrelated_dict_ids = torch.ones(B, T_W_L, dtype=torch.long) * self.tokenizer.pad_token_id
        ...
        for i, entry in enumerate(batch):
            ...
            if self.args.use_vision:
                ...
                # tags[i, 0] = entry['tag']  # Store in batch tensor

                # If tag == 1, disable image attention
                if entry['tag'] == 0:
            ...
            if 'tag' in entry:
                ids.append(entry['tag'])

            if 'unrelated_dict_ids' in entry:
                unrelated_dict_ids[i, :entry['unrelated_dict_length']] = entry['unrelated_dict_ids']
            ...
            if 'unrelated_dict' in entry:
                unrelated_dicts.append(entry['unrelated_dict'])
        ...
        batch_entry['id'] = ids
        batch_entry['tag'] = tag_list
        if 'target_ids' in batch[0]:
            word_mask = target_ids != self.tokenizer.pad_token_id
            target_ids[~word_mask] = -100
            batch_entry['target_ids'] = target_ids

        if any('unrelated_dict_ids' in entry for entry in batch):  # Check all entries
            word_mask = unrelated_dict_ids != self.tokenizer.pad_token_id
            unrelated_dict_ids[~word_mask] = -100
            batch_entry['unrelated_dict_ids'] = unrelated_dict_ids
        ...
        batch_entry['unrelated_dict'] = unrelated_dicts
...
class GREvaluator:
    ...
    def evaluate(self, predicts, answers):
        ...
        for i in range(len(predicts)):
            ...
            dcg1 = sum([1 / math.log2(idx + 2) for idx, item in enumerate(predict[:1]) if item in answer])
            dcg3 = sum([1 / math.log2(idx + 2) for idx, item in enumerate(predict[:3]) if item in answer])
            dcg5 = sum([1 / math.log2(idx + 2) for idx, item in enumerate(predict[:5]) if item in answer])

            # IDCG@K
            idcg1 = sum([1 / math.log2(idx + 2) for idx in range(min(1, len(answer)))])
            idcg3 = sum([1 / math.log2(idx + 2) for idx in range(min(3, len(answer)))])
            idcg5 = sum([1 / math.log2(idx + 2) for idx in range(min(5, len(answer)))])

            # Normalize DCG by IDCG to get NDCG
            ndcg1 += dcg1 / idcg1 if idcg1 > 0 else 0
            ndcg3 += dcg3 / idcg3 if idcg3 > 0 else 0
            ndcg5 += dcg5 / idcg5 if idcg5 > 0 else 0

            # MRR
            for rank, item in enumerate(predict):
                if item in answer:
                    mrr += 1 / (rank + 1)
                    break
        ...
        ndcg1 = ndcg1 / len(predicts)
        ndcg3 = ndcg3 / len(predicts)
        ndcg5 = ndcg5 / len(predicts)
        mrr = mrr / len(predicts)
            # break
