...
class GRFineTuneDataset(Dataset):
    def __init__(self, split='train', raw_dataset=None, rank=-1, topk=-1, verbose=True, args=None, mode='train'):
        ...
        data_info_path = dataset_dir.joinpath(f'./Final/First_turn_query/{source}_dataset_bert_first_turn.json')
        n_images = 0
                ...
                for k in karpathy_data:
                    ...
                    new_datum = {
                        'img_ids': [
                            k['img_ids1'][0] ,
                            # k['img_ids2'][0] ,
                            # k.get('img_ids3', [None])[0] , 
                            # k.get('img_ids4', [None])[0]
                        ],
                        'id': k['facet_id'],
                        'sent': (
                            k['topic'] + ' ' +
        ...
        # print(f'{dataset_dir}/keywords_output_new.pkl')
    ...
    def collate_fn(self, batch):
        ...
        if any('target_ids' in entry for entry in batch):  # Check all entries
            word_mask = target_ids != self.tokenizer.pad_token_id
            target_ids[~word_mask] = -100
            batch_entry['target_ids'] = target_ids
...
#     cudnn.benchmark = True
    ...
    for i, batch in enumerate(train_loader):
        ...
        unrelated_dicts.extend(batch['unrelated_dict'])
        # print(batch['targets'])
        # print(batch['input_text'][0])
        # print(batch['targets'][0])
        # break
    print(len(targets))
    print(len(unrelated_dicts))
