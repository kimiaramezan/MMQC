...
vg_dir = dataset_dir
img_dir = '../../Bert_datas/img_feature_all.pkl'
...
class GRFineTuneDataset(Dataset):
    def __init__(self, split='train', raw_dataset=None, rank=-1, topk=-1, verbose=True, args=None, mode='train'):
        ...
        source = self.source
        data_info_path = dataset_dir.joinpath(f'{source}_dataset_bert.json')
        n_images = 0
        with open(data_info_path) as f:
            ...
            if source == 'train':
                for k in karpathy_data:
                    print(k['img_ids1'][0])
                    new_datum = {
                        'img_ids': [k['img_ids1'][0] , k['img_ids2'][0] , k.get('img_ids3', [None])[0] , k.get('img_ids4', [None])[0]],
                        'id': k['facet_id'],
                        'sent': (
                            k['topic'] + ' ' +
        ...
        # self.source_to_h5 = {}
        self.dict_topic = pickle.load(open(f'{dataset_dir}/keywords_output.pkl','rb'))
    ...
    def __getitem__(self, idx):
        ...
        if 'targets' in datum:
            ...
            out_dict['targets'] = ' [SEP] '.join([self.dict_topic[t] for t in datum['targets'] if t in self.dict_topic])
        if datum['is_train']:
