...
def train_iteration(model, optimizer, dataset, img_pairs, img_embed_dict, img_tag_dict, train_pairs, qrels,use_img=True):
    ...
    with tqdm('training', total=BATCH_SIZE * BATCHES_PER_EPOCH, ncols=80, desc='train', leave=False) as pbar:
        for record in data.iter_train_pairs(model, dataset, img_pairs, img_embed_dict, img_tag_dict, train_pairs, qrels, GRAD_ACC_SIZE):
            if use_img:
...
def validate(model, dataset, img_pairs, img_embed_dict, img_tag_dict, run, valid_qrels, use_img=True):
    ...
    primary_metric = 'P_5'
    metrics = {primary_metric, 'P_1', 'P_3', 'ndcg_cut_1', 'ndcg_cut_3', 'ndcg_cut_5' , 'recip_rank'}
    ...
    primary_metric_avg = sum(
        averaged_scores[qid][primary_metric] for qid in averaged_scores if primary_metric in averaged_scores[qid]
    ) / len(averaged_scores)
...
def run_model(model, dataset, img_pairs, img_embed_dict, img_tag_dict, run, use_img=True, desc='valid'):
    ...
    with torch.no_grad(), tqdm(total=sum(len(run[qid]) * len(ds_queries[qid]) for qid in run), ncols=80, desc=desc, leave=False) as pbar:
        
        model.eval()
        for records in data.iter_valid_records(model, dataset, img_pairs, img_embed_dict, img_tag_dict, run, BATCH_SIZE):
            if use_img:
...
def main_cli():
    ...
    # img_pairs = data.read_img_dict(args.img_dict)
    qrels = data.read_qrels_dict(args.qrels)
    train_pairs = data.read_pair_dict(args.train_pairs)
